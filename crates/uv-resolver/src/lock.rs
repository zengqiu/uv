    ...
    fn to_dist(&self, workspace_root: &Path, tags: &Tags) -> Result<Dist, LockError> {
        if let Some(best_wheel_index) = self.find_best_wheel(tags) {
            return match &self.id.source {
                ...
                Source::Git(_, _) => Err(LockError::invalid_wheel_source(self.id.clone(), "Git")),
                Source::Directory(_) => Err(LockError::invalid_wheel_source(
                    self.id.clone(),
                    "directory",
                )),
                Source::Editable(_) => {
...
impl std::str::FromStr for Source {
    ...
    fn from_str(s: &str) -> Result<Source, SourceParseError> {
        let (kind, url_or_path) = s
            .split_once('+')
            .ok_or_else(|| SourceParseError::no_plus(s))?;
        match kind {
            "registry" => {
                let url =
                    Url::parse(url_or_path).map_err(|err| SourceParseError::invalid_url(s, err))?;
...
impl LockError {
    ...
    fn verbatim_url(id: DistributionId, error: VerbatimUrlError) -> LockError {
        let kind = LockErrorKind::VerbatimUrl { id, err: error };
        LockError {
            kind: Box::new(kind),
        }
    }
...
impl std::error::Error for LockError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match *self.kind {
            ...
            LockErrorKind::VerbatimUrl { ref err, .. } => Some(err),
        }
    }
}
...
impl std::fmt::Display for LockError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match *self.kind {
            ...
            LockErrorKind::VerbatimUrl { ref id, .. } => {
                write!(f, "found distribution {id} with invalid path or URL")
            }
        }
    }
}
